...
- 待精修：~~月相改 SVG 灰階~~ ✅Session 11、~~節氣/農曆~~ ✅Session 11；sw.js 快取版本號已於 Session 7 改 network-first。
...
## 其他點子
- ✅ **月相 / 節氣 / 農曆顯示**（Session 11）：手帳日期區塊顯示農曆日期＋當前/當日節氣；月相改灰階莫蘭迪 SVG（真實相位，多邊形終止線）。農曆用 jjonline `lunarInfo` 表(1900-2100)，節氣用壽星公式(2000-2099 準)。
- ✅ **主題切換（莫蘭迪 / 宣紙 / 夜間）**（Session 11）：設定面板三鈕，`gx_theme` 持久化，`html.theme-*` 覆寫 CSS 變數＋`<head>` 前置 script 防白閃；`--seal` 月色照樣每月變；夜間時手帳頁維持紙感（`.hobo` 子樹重宣告淺色變數）。
- ✅ **朗讀語速 / 語音選擇**（Session 11）：設定面板語速滑桿＋中文語音選單，`gx_ttsrate`/`gx_ttsvoice` 持久化（裝置本地，不雲端同步）。
- ✅ 雲端同步（Session 10，Firebase 三裝置 Google 登入）。
- 主題：可考慮加「自動跟隨系統深淺色」（`prefers-color-scheme`）。
